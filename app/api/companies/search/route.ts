...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (countryCode) {
            ...
            // Standard search by name or slug
            const formattedQuery = query.trim().replace(/\s+/g, "*")
            url = `${SUPABASE_URL}/rest/v1/companies?select=id,name,slug,website_url,country_code,categories(name,slug)&or=(name.ilike.*${encodeURIComponent(formattedQuery)}*,slug.ilike.*${encodeURIComponent(formattedQuery)}*)&limit=50`
        }
