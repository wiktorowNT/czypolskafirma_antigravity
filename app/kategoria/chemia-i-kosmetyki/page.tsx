...
export default async function ChemiaKosmetykiPage() {
  try {
    const categoryData = await getCachedCategoryData("chemia-kosmetyki")
    ...
    const category = {
      slug: "chemia-kosmetyki",
      name: "Kosmetyki i chemia domowa",
      short: "Kosmetyki, środki czystości i higiena",
      items: categoryData.companies,
    }

    console.log(`[v0] ChemiaKosmetykiPage: Loaded ${categoryData.companies.length} companies from Supabase`)
    ...
    console.error("[v0] Error in ChemiaKosmetykiPage:", error)
    return (
      <div className="min-h-screen bg-slate-50">
        <ScrollToTop />
