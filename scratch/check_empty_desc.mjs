...
async function main() {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/companies?select=id,name,slug,business_description`,
    {
      headers: {
  ...
  const empty = companies.filter(c => !c.business_description || c.business_description.trim() === '');
  console.log(`Found ${empty.length} companies with empty business_description:`);
