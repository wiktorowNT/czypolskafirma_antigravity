...
const SOURCES = [
  { name: 'icon.horse', getUrl: (domain) => `https://icon.horse/icon/${domain}` },
  { name: 'Google Favicon V2', getUrl: (domain) => `https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=http://${domain}&size=512` },
  { name: 'Favicone', getUrl: (domain) => `https://favicone.com/${domain}?s=512` },
  { name: 'DuckDuckGo', getUrl: (domain) => `https://external-content.duckduckgo.com/ip3/${domain}.ico` }
];

function fetchBuffer(url) {
  return new Promise((resolve) => {
    const client = url.startsWith('https') ? https : http;
    const req = client.get(url, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' }, timeout: 10000 }, (res) => {
      ...
      res.on('end', () => { resolve({ buffer: Buffer.concat(chunks), contentType, size: Buffer.concat(chunks).length }); });
...
async function processDomain(domain) {
  for (const source of SOURCES) {
    try {
      const result = await fetchBuffer(source.getUrl(domain));
...
async function main() {
  console.log(`🚀 Szybkie pobieranie dla ${domains.length} firm...`);
  
  const CONCURRENT_LIMIT = 5;
  for (let i = 0; i < domains.length; i += CONCURRENT_LIMIT) {
    const batch = domains.slice(i, i + CONCURRENT_LIMIT);
